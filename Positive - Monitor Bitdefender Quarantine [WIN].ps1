...
$SQLLitePresent = Test-Path $SQLLitePath
$alertThreshold = (Get-Date).AddDays(-1)
$diagMessages = @()

if (!$SQLLitePresent) {
    try {
        $DownloadUrl = "https://system.data.sqlite.org/blobs/1.0.119.0/sqlite-netFx46-binary-bundle-x64-2015-1.0.119.0.zip"
        $TempZipFile = "$env:TEMP\sqlite-netFx46-binary-bundle-x64.zip"
        $ExtractPath = "$env:TEMP\SQLite"
        $SQLiteDllPath = Join-Path $ExtractPath "System.Data.SQLite.dll"
        ...
        if (-not (Test-Path $SQLiteDllPath)) {
            write-DRMMDiag "ERROR: No SQLite DLL. Please run 'Positive - Add Bitdefender Quarantine Reader [WIN]' to add the DLL file."
            write-DRMMAlert "ERROR: No SQLite DLL. Please run 'Positive - Add Bitdefender Quarantine Reader [WIN]' to add the DLL file."
            exit 1
        } else {
            Copy-Item -Path $SQLiteDllPath -Destination $SQLLitePath
        }
    } catch {
        write-DRMMDiag "ERROR: No SQLite DLL. Please run 'Positive - Add Bitdefender Quarantine Reader [WIN]' to add the DLL file."
        write-DRMMAlert "ERROR: No SQLite DLL. Please run 'Positive - Add Bitdefender Quarantine Reader [WIN]' to add the DLL file."
